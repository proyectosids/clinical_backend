...
class MssqlCitaRepository extends CitaRepository {
  async create(cita) {
    ...
    const colsRes = await pool
      .request()
      .query(
        `SELECT COLUMN_NAME, IS_NULLABLE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'Cita'`,
      );
    const columns = new Set(
      (colsRes.recordset || []).map((r) => r.COLUMN_NAME),
    );
    ...
    if (usesHorario) {
      ...
      if (missing.length) {
        throw new Error(
          `REQUIRED_FIELDS_MISSING: faltan campos obligatorios para este esquema de Cita: ${missing.join(", ")}`,
        );
      }
      ...
      insertCols.push("id_servicio");
      insertVals.push("@id_servicio");
      insertCols.push("id_profesional");
      insertVals.push("@id_profesional");
      insertCols.push("fecha_cita");
      insertVals.push("@fecha_cita");
      insertCols.push("hora_cita");
      insertVals.push("@hora_cita");
      insertCols.push("id_estado_cita");
      insertVals.push("@id_estado_cita");
      if (hasMotivoDetalle) {
        insertCols.push("motivo_detalle");
        insertVals.push("@motivo_detalle");
      }
      ...
      if (hasMotivoDetalle)
        req.input("motivo_detalle", cita.motivo_detalle || null);
    ...
    const result = await pool
      ...
      .input("id_servicio", cita.id_servicio)
      .input("id_usuario", cita.id_usuario).query(`
  ...
  async update(id, data) {
    ...
    const colsRes = await pool
      .request()
      .query(
        `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'Cita'`,
      );
    const columns = new Set(
      (colsRes.recordset || []).map((r) => r.COLUMN_NAME),
    );
    ...
    if (!usesHorario) {
      ...
      await pool
        ...
        .input("id_usuario", data.id_usuario).query(`
    ...
    if (hasMotivoDetalle)
      req.input("motivo_detalle", data.motivo_detalle || null);
