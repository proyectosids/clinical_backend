...
class MssqlGinecoObstetriciaRepository extends GinecoObstetriciaRepository {
  async create(gineco) {
    ...
    const result = await pool
      ...
      .input(
        "dismenorrea",
        typeof gineco.dismenorrea !== "undefined" ? gineco.dismenorrea : 0,
      )
  ...
  async findAll() {
    ...
    const result = await pool
      .request()
      .query("SELECT * FROM Gineco_Obstetricia");
  ...
  async findByPaciente(id_paciente) {
    ...
    const result = await pool
      ...
      .query(
        "SELECT * FROM Gineco_Obstetricia WHERE id_paciente = @id_paciente ORDER BY id_gineo DESC",
      );
    return result.recordset;
  }
  ...
  async update(id, data) {
    ...
    await pool
      ...
      .query(
        "UPDATE Gineco_Obstetricia SET menarca=@menarca, fum=@fum, ivsa=@ivsa, dismenorrea=@dismenorrea, embarazos=@embarazos, partos=@partos, cesareas=@cesareas, abortos=@abortos WHERE id_gineo=@id_gineo",
      );
    return true;
  }
