...
class MssqlNotaMedicaSeguimientoRepository extends NotaMedicaSeguimientoRepository {
  async create(nota) {
    ...
    if (!id_paciente) {
      throw new Error(`No se pudo determinar id_paciente para la nota. Verifica que la cita (id_cita=${nota.id_cita}) exista y tenga id_paciente.`);
    }
    ...
    const result = await pool.request()
      ...
      .input('fecha_hora_atencion', nota.fecha_hora_atencion)
      .query(`
      ...
      `);
    return result.recordset[0] && result.recordset[0].id_nota_medica_seguimiento;
  }
  ...
  async update(id, data) {
    ...
    await pool.request()
      ...
      .input('fecha_hora_atencion', data.fecha_hora_atencion)
      .query(`
  ...
  async delete(id) {
    ...
    await pool.request()
      .input('id_nota_medica_seguimiento', id)
      .query(`DELETE FROM NotaMedicaSeguimiento WHERE id_nota_medica_seguimiento = @id_nota_medica_seguimiento`);
    return true;
  }
}
