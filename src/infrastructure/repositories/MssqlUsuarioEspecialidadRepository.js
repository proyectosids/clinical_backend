...
class MssqlUsuarioEspecialidadRepository {
  ...
  async addMany(id_usuario, especialidades = []) {
    if (!Array.isArray(especialidades) || especialidades.length === 0) return true;
    const pool = await getPool();
    ...
    try {
      ...
      for (let i = 0; i < especialidades.length; i++) {
        ...
        await trReq.query(`INSERT INTO usuarioEspecialidad (id_usuario, id_especialidad) VALUES (@id_usuario_${i}, @id_especialidad_${i})`);
      }
  ...
  async remove(id_usuario, id_especialidad) {
    ...
    await pool.request()
      .input('id_usuario', id_usuario)
      .input('id_especialidad', id_especialidad)
      .query('DELETE FROM usuarioEspecialidad WHERE id_usuario = @id_usuario AND id_especialidad = @id_especialidad');
    return true;
  }
