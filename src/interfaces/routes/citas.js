...
router.post("/crear-con-horario", async (req, res) => {
  const { id_paciente, id_medico, id_servicio, fecha, hora, id_profesional } =
    req.body || {};
  if (!id_medico || !id_servicio || !fecha || !hora) {
    return res
      .status(400)
      .json({
        error: "Faltan campos requeridos: id_medico, id_servicio, fecha, hora",
      });
  }
  try {
    ...
    const horaColRes = await pool
      .request()
      .query(
        "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'Horario' AND COLUMN_NAME IN ('hora','hora_inicio')",
      );
    const horaCol =
      (horaColRes.recordset &&
        horaColRes.recordset[0] &&
        horaColRes.recordset[0].COLUMN_NAME) ||
      "hora";
    // Detectar la columna de profesional en la tabla Cita
    const colsRes = await pool
      .request()
      .query(
        "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'Cita' AND COLUMN_NAME IN ('id_medico','id_profesional','id_usuario')",
      );
    const profCol =
      (colsRes.recordset &&
        colsRes.recordset[0] &&
        colsRes.recordset[0].COLUMN_NAME) ||
      "id_profesional";
    // Detectar si Cita tiene id_clinica
    const hasClinicaRes = await pool
      .request()
      .query(
        "SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'Cita' AND COLUMN_NAME = 'id_clinica'",
      );
    const hasIdClinica = !!(
      hasClinicaRes.recordset && hasClinicaRes.recordset.length
    );
    ...
    try {
      ...
      if (!horario) {
        ...
        return res
          .status(404)
          .json({
            error: "No hay horario disponible en la fecha/hora solicitada",
          });
      }
      ...
      await trReq
        ...
        .query(
          `UPDATE Horario SET status = 2, id_cita = NULL WHERE id_horario = @id_horario`,
        );
      ...
      const insertReq = trReq
        .input("id_paciente", id_paciente || null)
        .input("id_horario", horario.id_horario)
        .input("id_servicio", id_servicio)
        .input("id_profesional", id_profesional || id_medico || null)
        .input("fecha_cita", fecha)
        .input("hora_cita", hora)
        .input("id_estado_cita", 1);
      if (hasIdClinica)
        insertReq.input("id_clinica", horario.id_clinica || null);
      const insertRes = await insertReq.query(insertSql);
      const id_cita =
        insertRes.recordset &&
        insertRes.recordset[0] &&
        insertRes.recordset[0].id_cita;
      // Actualizar horario para asociar la cita y marcar reservado
      await trReq
        .input("id_horario", horario.id_horario)
        .input("id_cita", id_cita)
        .query(
          `UPDATE Horario SET status = 2, id_cita = @id_cita WHERE id_horario = @id_horario`,
        );
