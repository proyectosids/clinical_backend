...
const getGinecoByPacienteUseCase = new GetGinecoObstetriciasByPacienteUseCase(ginecoRepo);
...
router.post('/paciente/:id_paciente', (req, res, next) => {
  ...
  req.body = Object.assign({}, req.body, { id_paciente: parseInt(req.params.id_paciente, 10) });
  const { error } = ginecoObstetriciaSchema.validate(req.body);
  if (error) return res.status(400).json({ error: error.details[0].message });
  ginecoController.create(req, res, next);
});
router.get('/', (req, res) => ginecoController.getAll(req, res));
router.get('/paciente/:id_paciente', (req, res) => ginecoController.getByPaciente(req, res));
